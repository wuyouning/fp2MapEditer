...
export class CustomSlider2 {
    ...
    render() {
        ...
        sliderContainer.appendChild(this.decreaseButton);
        ...
        this.slider.addEventListener("input", (event) => this.updateValue(event));
        ...
        sliderContainer.appendChild(this.increaseButton);

            // 为按钮添加点击事件
        this.increaseButton.addEventListener("click", () => {
            this.value = Math.min(this.value + this.step, this.max);  // 增加值但不能超过最大值
            this.updateSlider();
            if (this.onChangeCallback) this.onChangeCallback(this.value);
        });

        this.decreaseButton.addEventListener("click", () => {
            this.value = Math.max(this.value - this.step, this.min);  // 减少值但不能小于最小值
            this.updateSlider();
            if (this.onChangeCallback) this.onChangeCallback(this.value);
        });
    ...
    decreaseValue() {
        let value = parseInt(this.slider.value, 10);
    ...
    updateSlider() {
        // 假设 slider 已经渲染出来并且是通过 width 反映值的
        const sliderElement = this.container.querySelector('.slider');
        if (sliderElement) {
            const percentage = (this.value - this.min) / (this.max - this.min) * 100;
            sliderElement.style.width = `${percentage}%`;  // 更新滑块的宽度
        }
    }
}
